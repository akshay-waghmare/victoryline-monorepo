...
- [ ] T028 **PENDING (post-deploy)**: Run the SEO health pattern audit: `powershell -ExecutionPolicy Bypass -File .\.agents\skills\crickzen-seo-health-pattern-audit\scripts\Audit-CrickzenSeoHealth.ps1` and confirm `h1=2` failures are zero and the audit exit code is 0 — requires frontend deploy first
- [ ] T029 [P] **PENDING (post-deploy)**: Inspect raw SSR HTML of one completed match page: `curl -s https://www.crickzen.com/cric-live/<completed-slug> | Select-String -Pattern '<h1'` and assert exactly one match — requires frontend deploy first
- [ ] T030 [P] **PENDING (post-deploy)**: Inspect raw SSR HTML of one upcoming match page: extract `application/ld+json` blocks and assert a `SportsEvent` block with `startDate` and `eventStatus` is present (location may be absent) — requires frontend deploy first
- [ ] T031 [P] **PENDING (post-deploy)**: Validate the extracted `SportsEvent` JSON-LD with the Schema.org validator (`https://validator.schema.org/`) and confirm it passes with no errors — requires frontend deploy first
- [ ] T032 **PENDING (post-deploy, manual)**: Manually run the Google Rich Results Test (`https://search.google.com/test/rich-results`) on one pre-match production URL post-deploy and document the rich-result eligibility result (cannot be automated; JS-rendered tool)
...
- [ ] T034 **PENDING (post-deploy)**: Confirm `/cric-live/{slug}` canonical policy and Spec 023 behavior are unchanged (no new routes, no canonical migration, no alias changes)
- [x] T035 **PENDING**: Roll out backend changes via the `crickzen-backend-scraper-prod-rollout` skill and frontend changes via the `crickzen-frontend-prod-rollout` skill; rerun the audit after each rollout — **Backend rollout DONE (2026-06-25 09:13 UTC)**: image `victoryline-backend:030-prematch-seo` deployed, healthy, US1/US4/US5 verified in prod. Frontend rollout (US2/US3) still pending.
